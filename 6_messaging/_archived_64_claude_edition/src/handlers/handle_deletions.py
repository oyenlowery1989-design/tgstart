...
"""
FILE: src/handlers/forensics.py -> src/handlers/handle_deletions.py
PURPOSE: Background task that detects when messages are deleted.
"""
import os
import asyncio
                    ...
                    for i, m_obj in enumerate(results):
                        ...
                        if is_deleted:
                            ...
                            if orig_id in pair.msg_map:
                                entry = pair.msg_map[orig_id]
                                
                                # Handle both old (tuple) and new (dict) formats
                                if isinstance(entry, dict):
                                    mirrored_msg_id = entry.get('id')
                                    sender = entry.get('sender', 'Unknown')
                                    old_text = entry.get('text', '')
                                else:
                                    mirrored_msg_id = entry[0].id
                                    sender = entry[1]
                                    old_text = entry[2]
                                ...
                                if NOTIFY_DELETIONS:
                                    ...
                                    try:
                                        await self.client.send_message(pair.dest_entity, alert, reply_to=mirrored_msg_id)
