...
"""
FILE: src/handlers/handle_messages.py
PURPOSE: Handles Message Mirroring (New Messages & Edits)
IMPROVED: Smart Reply Handling (Forward vs Copy)
"""
import os
import traceback
...
from src.core.user_intelligence import record_user_seen_sender, record_user_seen_mention
    ...
    async def register(self):
            ...
            try:
                ...
                if event.message.reply_to:
                    ...
                    if parent_id in pair.msg_map:
                        reply_to_dst_id = pair.msg_map[parent_id]['id']
                    else:
                        reply_context_msg = f"↪️ **Replying to old/missing message**"

                # Mirror Logic with improved media handling & Smart Replies
                mirrored = None
                try:
                    if reply_to_dst_id:
                        # SMART REPLY LOGIC:
                        # If a message is a reply, we CANNOT use forward_messages + reply_to.
                        # We must COPY the message (send_message/send_file) with the reply_to ID.
                        
                        # Add attribution header
                        header = f"👤 **Forwarded from:** @{sender_name}\n\n"
                        msg_text = event.message.message or ""
                        
                        # Copy Logic
                        if event.message.media:
                            # Send File with caption
                            mirrored = await self.client.send_file(
                                pair.dest_entity, 
                                event.message.media, 
                            ...
                            # Send Text
                            mirrored = await self.client.send_message(
                                pair.dest_entity, 
                                f"{header}{msg_text}", 
                                reply_to=reply_to_dst_id,
                                link_preview=False
                        ...
                        # If NOT a reply, use standard forward to preserve "Forwarded from" UI element
                        mirrored = await self.client.forward_messages(pair.dest_entity, event.message)
                
                except FloodWaitError as e:
                    await self.handle_flood_wait(e, "message_mirror")
                    # Retry logic (simplified)
                    try:
                        mirrored = await self.client.forward_messages(pair.dest_entity, event.message)
                    except Exception as e2:
                        log_error("message_mirror_retry", str(e2))
                except Exception as e:
                    log_error("message_mirror", str(e))
                    try:
                        # Fallback to simple send_message
                ...
                if mirrored_msg:
                    if reply_context_msg:
                        try:
                            # If we missed the reply due to cache eviction, comment on it
        ...
        async def edit_handler(event):
            ...
            try:
                ...
                # Support both old (tuple) and new (dict) formats during transition
                if isinstance(orig_msg_data, dict):
                    reply_to_id = orig_msg_data.get('id')
                    old_text = orig_msg_data.get('text', "")
                else:
                ...
                # Update cache
                # Update cache using the new persistent format
                if orig_msg_data:
                    pair.msg_map[event.message.id] = {
                        "id": reply_to_id,
                        "sender": sender_name,
                        "text": new_text
                    }
