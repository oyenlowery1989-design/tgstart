...
"""
FILE: src/handlers/tracking.py -> src/handlers/handle_members.py
PURPOSE: Manages User Movement (Joins, Leaves, Kicks, Adds) and Invite Links.
"""
import os
import traceback
...
from src.core.user_intelligence import record_user_seen_sender
                ...
                if event.user_joined or event.user_added:
                    ...
                    if is_bot:
                        ...
                        if admin_name and event.user_added:
                            ...
                            if inviter_name:
                                if inviter_name == "Anonymous Admin":
                                    action_text = f"🔗 **User Joined via Link:** @{sender_name}"
                                else:
        ...
        if UpdateChatInviteImporter:
            async def invite_join_handler(event):
                try:
                    ...
                    # Build alert
                    if created_by:
                        action_text = f"🔗 **User Joined via Invite:** @{user_name} (link by @{created_by})"
                    else:
